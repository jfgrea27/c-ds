...
- [[cvector]]: a dynamic array of pointers.
- [[clinklist]]: a linked list.
- [[cstack]]: a stack backed by [[cvector]].
- [[cqueue]]: a queue backed by [[clinklist]].
- [[chashmap]]: a hash map that uses [[clinklist]] for collision chaining.
- [[cbst]]: a binary search tree.
...
*Implementation*: Uses [[cvector]] as the underlying storage. Push/pop operate on the end of the vector for O(1) performance.

A full example of the `CStack` API in action can be seen in `src/cstack/main.c`.
...
*Implementation*: Uses [[clinklist]] as the underlying storage. Enqueue appends to the list, dequeue removes from the front (FIFO).

A full example of the `CQueue` API in action can be seen in `src/cqueue/main.c`.
...
*Implementation*: Uses an array of [[clinklist]] buckets for collision chaining. Requires user-provided hash and equality functions.

A full example of the `CHashMap` API in action can be seen in `src/chashmap/main.c`.
...
A binary search tree with user-provided comparison function. Supports insert, search, delete, min/max operations.

A full example of the `CBST` API in action can be seen in `src/cbst/main.c`.
